...
export class ShopMainPageComponent {
 previewURLs: string[] = []; // Список загруженных фото
  ...
  public productMinPrice: ReturnProductListDto;
  public productMaxPrice: ReturnProductListDto;
  ...
  ngOnInit() 
  {
    ...
    this.shopService.getAllProducts().subscribe({
      next: (data) => {
        ...
        this.products = data;
        if (data.length > 0) {
          this.productMinPrice = data.reduce((min, product) => product.price < min.price ? product : min);
          this.productMaxPrice = data.reduce((max, product) => product.price > max.price ? product : max);
        }
      },
      error: (err) => {
        console.error('Ошибка загрузки продукты', err);
      }
    });
  ...
  addToCart(productId: string): void {
    this.shopService.addToCart(productId, 1);
  }
  onCategoryChange(isChecked: boolean, categoryId: string): void {
    if (isChecked) {
  ...
  applyFilters(): void {
    this.products = this.allProducts.filter(product => {
      const inCategory = this.selectedCategoryIds.length === 0 || this.selectedCategoryIds.includes(product.categoryId);
      const inPriceRange = product.price >= this.progressValue;
  ...
  products2 = [
    {
      image: 'https://vip-print.ua/upload/iblock/362/8722-02-01.webp?1693296316',
      title: 'Чашка Seed Flow',
      description: '450 мл',
      price: '450 грн', 
      leftTopImage: 'assets/images/ShopGreenLeftUp2.png', 
      rightBottomImage: 'assets/images/ShopKorzina.png',
      leftTopText: 'Дім',
      rightTopImage: 'assets/images/like.png'
    },
    {
    ...
    {
      image: 'assets/images/Bloknot.png',
      title: 'Книга Seed Flow',
      description: '300 стр',
      price: '150 грн',
      leftTopImage: 'assets/images/ShopGreenLeftUp2.png', 
      rightBottomImage: 'assets/images/ShopKorzina.png',
    ...
    {
      image: 'assets/images/ShopTovar4.png',
      title: 'Шапка Seed Flow',
  ...
  toggleLike(id: string, i: number) {

    this.shopService.toggleLike(id).subscribe({
      next: () => {
        this.products[i].isFavourite = !this.products[i].isFavourite;
      },
      error: (error) => {
