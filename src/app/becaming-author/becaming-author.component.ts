...
@Component({
  ...
  templateUrl: './becaming-author.component.html',
  styleUrl: './becaming-author.component.scss'
})
export class BecamingAuthorComponent {
carouselView: TemplateRef<NgIfContext<boolean>>;
  ...
  toggleLike(index: number): void {
    ...
    isFormValid(): boolean {
      return this.isFullNameValid() && this.isPasswordValid();
    }
  
    // Проверка ПІБ
    isFullNameValid(): boolean {
      const fullNamePattern = /^[А-Яа-яЁёІіЇїЄєҐґA-Za-z'’\-]+ [А-Яа-яЁёІіЇїЄєҐґA-Za-z'’\-]+ [А-Яа-яЁёІіЇїЄєҐґA-Za-z'’\-]+$/;
      return fullNamePattern.test(this.name);
    }
  
    // Проверка пароля
    isPasswordValid(): boolean {
      return this.password.trim().length > 0;  // Пароль не пустой
    }
    ...
    sendMessage(): void {
      // Проверка валидности формы
      if (!this.isFormValid()) {
        this.errorMessage = 'Будь ласка, заповніть усі поля коректно!';
        return; // Остановим выполнение, если форма не валидна
      }
  
      console.log('Message sent:', this.name, this.password);
  
      // Ваш код для отправки данных
      this.userService.becomeAuthor(this.password).subscribe({
        next: (response) => {
          console.log('Заявка на авторство успешно отправлена:', response);
          this.authService.setToken(response.token); 
          this.messageSent = true;
        },
        error: (error) => {
          // Обработка ошибок
          this.errorMessage = error?.error?.message || 'Паролі не співпадають';
        }
      });
    }
}
