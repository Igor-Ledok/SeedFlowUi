...
@Component({
  ...
  templateUrl: './comment-donation.component.html',
  styleUrl: './comment-donation.component.scss'
})
export class CommentDonationComponent {
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      console.log('Получен projectId:', projectId);
      console.log('Получена сумма доната:', sum);
      this.sum = sum;
  ...
  sendDonationWithComment(){
    if (this.supportText) {
      const donate: DonationRequest = { sum: this.sum, message: this.supportText };
      this.projectService.donateToProject(this.project.id, donate).subscribe({
        next: (response) => {
          console.log('Donation successful:', response);
          this.router.navigate(['/comment-donation-access-page'], { queryParams: { projectId: this.project.id } });
        },
        error: (error) => {
  ...
  sendDonation(){
    if (!this.supportText) {
      const donate: DonationRequest = { sum: this.sum, message: null };
      this.projectService.donateToProject(this.project.id, donate).subscribe({
        next: (response) => {
          console.log('Donation successful:', response);
          this.router.navigate(['/without-comment-donation-access-page'], { queryParams: { projectId: this.project.id } });
        },
        error: (error) => {
          console.error('Error during donation:', error);
        },
